...
Windows Vista 부터는 드라이버를 로드하기 위해서 드라이버 파일에 디지털 서명을 적용해야 한다. 이 부분은 다소 다른 주제이기 때문에 드라이버 서명에 대한 우회 방법을 소개하는 정도로 그냥 넘어 가도록 한다. 이를 우회하기 위해서 “disable driver signature” 정도의 키워드로 검색해보고 해당 내용을 적용해야 할 것이다.
